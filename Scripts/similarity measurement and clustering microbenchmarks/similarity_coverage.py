...
def main() -> None:
    """
    Main function to generate and print a summary report of JMH benchmarks and their
    associated JU2JMH benchmarks with coverage >= 10%, sorted by overlap percentage,
    including throughput from ju2jmh.csv.
    """
    # Directory containing individual coverage reports.
    folder_path = "/Users/mj/workspace/RxJava/individual_coverage_reports/"
    # Path to the ju2jmh.csv file
    throughput_file = "/Users/mj/workspace/tmptmptmp/ju2jmh.csv"
    # Load throughput data
    throughput_data = load_throughput_data(throughput_file)
    # Generate the report
    generate_summary_report(folder_path, throughput_data)
...
def generate_summary_report(folder_path: str, throughput_data: ThroughputData) -> None:
    """
    Prints a summary report of JMH benchmarks and their associated JU2JMH benchmarks
    ...
    for jmh_folder in jmh_benchmarks:
        ...
        if matched_ju2jmh:
            print(f"> JMH Benchmark: {jmh_folder}")
            for ju2jmh_folder, overlap in matched_ju2jmh:
                throughput = throughput_data.get(ju2jmh_folder, "N/A")
                print(f" >> JU2JMH Benchmark: {ju2jmh_folder}, Overlap: {overlap:.2f}%, Throughput: {throughput}")
            print()
